...
export class ProductCreateComponent implements OnInit {
  ...
  onSubmit() {
    const product = new Product(this.productForm.value['name'], this.productForm.value['brand'], this.productForm.value['price'], null);
  ...
  private initForm() {
    this.productForm = new FormGroup({
      ...
      price : new FormControl(this.price, [Validators.required, Validators.pattern("^[0-9]*$")])
    });
  }
